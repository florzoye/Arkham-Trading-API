...
def db_row_to_account(row: dict) -> Account:
    ...
    return Account(
        ...
        proxy=row.get("proxy"),
        cookies=cookies,
        captcha_api=row.get("captcha_api")
    )
...
async def futures_menu(account: Account):
    while True:
        choice = await inquirer.select(
            message="Выберите действие:",
            choices=[
...
async def spot_menu(account: Account):
    while True:
        choice = await inquirer.select(
            message="Выберите действие:",
            choices=[
    ...
    try:
        ...
        for n in range(int(n_orders)):
            ...
            try:
                ...
                try:
                    set_leverage = await ArkhamLeverage(account.session).set_leverage(
...
async def remains_balance_orders(account: Account):
    try:
        console.print('[red]Торговая пара, направление и время сделки будут выбираться рандомно![/red]')
        ...
        point_balance = await inquirer.number(
            message="Сколько депозита оставить? (например: 50)",
        ).execute_async()
        ...
        while True:
            try:
                ...
                success = await (
                    trader.futures_long_limit() if side == "long" else trader.futures_short_limit()
                )
                ...
                if success:
                    ...
                    await close_all_positions(account)
                    successful_orders += 1
                    console.print(f"[green]✅ Все позиции закрыты, общее число позиций - {successful_orders}[/green]")

                    await show_basic_account_info(account)
                    await asyncio.sleep(delay)
                else:
                    console.print(f"[red]❌ Ошибка открытия позиции {coin}[/red]")
...
async def volume_orders(account: Account):
    try:
        console.print('[red]Торговая пара, направление и время сделки будут выбираться рандомно![/red]')
        ...
        add_volume = await inquirer.number(
            message='Какое количество объема вы желаете ДОБИТЬ? (например: 200000)'
        ).execute_async()

        add_volume = int(add_volume)

        # стартовый объем аккаунта
        start_volume = int(await account.arkham_info.get_perp_volume())
        goal_volume = start_volume + add_volume

        console.print(f"[cyan]📊 Текущий объем: {start_volume}, цель: {goal_volume}[/cyan]")

        while True:
            await account.update_data()
            current_volume = int(await account.arkham_info.get_perp_volume())

            if current_volume >= goal_volume:
                console.print(f"[green]✅ Достигнут целевой объем: {current_volume}/{goal_volume} [/green]")
                break
            ...
            if success:
                ...
                # пересчитываем реальный объем после закрытия
                new_volume = int(await account.arkham_info.get_perp_volume())
                console.print(f"[cyan]📊 Обновленный объем: {new_volume}/{goal_volume} [/cyan]")
...
async def open_position(account: Account, side: str):
    ...
    leverage_raw = await inquirer.text(
        message="Введите плечо для вашей сделки (1 - 20):"
    ).execute_async()
    ...
    try:
        ...
        if set_leverage == leverage:
            console.print(f"[green]✅ Плечо {leverage}x для {coin} успешно установлено[/green]")
        else:
            console.print(
                f"[yellow]⚠️ Не удалось поставить плечо. Используем дефолтное - {config.DEFAULT_LEVERAGE}[/yellow]"
            )
            leverage = config.DEFAULT_LEVERAGE

    except (TypeError, ValueError):
        console.print(
            f"[yellow]⚠️ Ошибка установки плеча. Используем дефолтное - {config.DEFAULT_LEVERAGE}[/yellow]"
        )
        leverage = config.DEFAULT_LEVERAGE
...
async def spot_balance(account: Account):
    try:
        balances = await account.arkham_info.get_spot_balances()
        if not balances:
            console.print("[yellow]⚠️ На спотовом кошельке нет активов[/yellow]")
            return

        table = Table(title="💰 Спотовый баланс")
        table.add_column("Монета", style="cyan")
        table.add_column("Количество", style="green")
        table.add_column("Цена в USDT", style='blue')

        for item in balances:
            table.add_row(item['symbol'], str(item['free']), item['balanceUSDT'])
        ...
        console.print(f"[red]❌ Ошибка получения спотового баланса: {e}[/red]")

async def spot_buy_coin(account: Account):
    coin = str(await inquirer.text(message="Введите монету (например BTC):").execute_async())

    # получаем цену
    price = dict(await account.arkham_price.get_spot_price(coin))['price']
    if not price:
        console.print(f"[red]❌ Не удалось получить цену {coin}[/red]")
        return
    ...
    if success:
        ...
        console.print(f"[red]❌ Ошибка покупки позиции[/red]")

async def spot_sell_coin(account: Account):
    try:
        coins = await account.arkham_info.get_spot_balances()
        choices = [coin['symbol'] for coin in coins]
    except Exception as e:
        console.print(f'[red]Ошибка получения балансов: {e}[/red]')
        return False

    try:
        choice = await inquirer.select(
            message='Выберите монету для продажи',
            choices=choices + ["❌ Отмена"],
        ).execute_async()
        ...
        try:
            size = float(next(coin['free'] for coin in coins if coin['symbol'] == choice))
        except StopIteration:
            console.print(f"[red]❌ Баланс для {choice} не найден[/red]")
            return False

        price_data = await account.arkham_price.get_spot_price(choice)
        price = price_data.get('price')
        if not price:
            console.print(f"[red]❌ Не удалось получить цену {choice}[/red]")
            return False
        ...
        success = await trader.spot_sell_market(sell_size=float(size))
        if success:
            console.print(f"[green]✅ Монета '{choice}' продана на споте![/green]")
        else:
            console.print(f"[red]❌ Ошибка продажи позиции[/red]")

    except Exception as e:
        console.print(f"[red]Ошибка при продаже монеты: {e}[/red]")
        return False

async def spot_volume_orders(account: Account):
    try:
        console.print('[red]Торговая пара и время сделки будут выбираться рандомно![/red]')
        ...
        add_volume = await inquirer.number(
            message='Какое количество объема вы желаете набить? (например: 200000)'
        ).execute_async()

        add_volume = int(add_volume)

        # стартовый объем аккаунта
        start_volume = int(await account.arkham_info.get_spot_volume())
        goal_volume = start_volume + add_volume
        ...
        while True:
            await account.update_data()
            current_volume = int(await account.arkham_info.get_spot_volume())
            ...
            if success:
                console.print(f"[green]✅ BUY {coin} на {size}[/green]")
                await asyncio.sleep(delay)

                sell_size = float(await account.arkham_info.get_spot_coin_free(coin.upper()))
                if sell_size > 0:
                    await trader.spot_sell_market(sell_size=sell_size)
                    console.print(f"[green]✅ SELL {coin} на {sell_size}[/green]")

                await asyncio.sleep(delay)
                successful_orders += 1

            else:
                console.print(f"[red]❌ Ошибка при покупке {coin}[/red]")
                action = await inquirer.number(
                    message="Возникла ошибка. Продолжить? (0 = да, 1 = выход):",
                ).execute_async()
                if action == 1:
                    break
                continue

        console.print(f"[green]🎯 Завершено {successful_orders} сделок[/green]")
...
async def add_account() -> Optional[Account]:
    ...
    try:
        ...
        captcha_api = await inquirer.text(message="Введите TwoCaptcha captcha_key:").execute_async()
        ...
        await save_account_to_db(account, points, volume, balance, margin_fee, margin_bonus, captcha_api)
...
async def save_account_to_db(
        ...
        captcha_api: Optional[float] | None = None,
    ):
...
async def login_arkham(account: Account) -> Optional[Account]:
    ...
    try:
        ...
        captcha = TwoCaptcha(session, account.captcha_api)
