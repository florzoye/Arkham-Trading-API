...
class ArkhamInfo:
    ...
    async def get_spot_volume(self):
        try:
            action = 'volume'
            url = f"https://arkm.com/api/affiliate-dashboard/{'volume' if action == 'volume' else 'points'}-season-2"
            async with self.session.get(url, headers=self.headers(action)) as response:
    ...
    async def get_spot_balances(self):
        """
        Получить все спотовые балансы (только free > 0).
        Возвращает список словарей с ключами: symbol, free, locked.
        """
        ...
        async with self.session.get(
            ...
            if response.status != 200:
                text = await response.text()
                logger.error(f"Не удалось получить спотовые балансы: {text}")
                return []

            data = await response.json()

            # может вернуться список или словарь с ключом "balances"
            ...
            return [bal for bal in balances if float(bal.get("free", 0)) > 0]

    async def get_spot_coin(self, coin: str) -> float:
        """
        Получить свободный баланс конкретной монеты на споте.
        """
        balances = await self.get_spot_balances()
        for bal in balances:
            if bal.get("symbol", "").upper() == coin.upper():
